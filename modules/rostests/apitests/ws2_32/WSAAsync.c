...
START_TEST(WSAAsync)
{
    ...
    {
        ok(closesocket(sockaccept) == 0, "Accept closesocket() failed: %d\n", WSAGetLastError());
        sockaccept = INVALID_SOCKET;
    }

    ok(closesocket(ClientSocket) == 0, "Client closesocket() failed: %d\n", WSAGetLastError());
    ClientSocket = INVALID_SOCKET;
    ok(closesocket(ServerSocket) == 0, "Server closesocket() failed: %d\n", WSAGetLastError());
    ServerSocket = INVALID_SOCKET;

    ok(WSACloseEvent(ClientEvent) == TRUE, "Client WSACloseEvent() failed: %d\n", WSAGetLastError());
    ClientEvent = WSA_INVALID_EVENT;
    ok(WSACloseEvent(ServerEvent) == TRUE, "Server WSACloseEvent() failed: %d\n", WSAGetLastError());
    ...
    ok(WSACleanup() == 0, "WSACleanup() failed: %d\n", WSAGetLastError());
}
