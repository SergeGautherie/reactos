...
#define print_supported(kf_value) ((FeatureBits & kf_value) ? " " #kf_value : "")
    DPRINT1("Supported CPU features :%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n",
            ...
            print_supported(KF_SSE3),
            print_supported(KF_CMPXCHG16B),
            print_supported(KF_XSTATE),
